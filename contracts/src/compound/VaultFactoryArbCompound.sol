...
import {RangeVaultArbCompound} from "./RangeVaultArbCompound.sol";

/// @title VaultFactoryArbCompound
/// @notice Fork of VaultFactoryArb.sol that clones RangeVaultArbCompound instead of
/// RangeVaultArb — kept as its own file so the existing VaultFactoryArb.sol (and
/// every vault already cloned from it) is never touched.
contract VaultFactoryArbCompound {
    error TokenMismatch();
    ...
    constructor(address _platformConfig, address _positionManager, address _swapRouter) {
        implementation = address(new RangeVaultArbCompound());
        ...
        returns (address vault)
    {
        ...
        RangeVaultArbCompound(vault).initialize(

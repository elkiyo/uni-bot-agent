...
/// @title RangeVaultArbCompound
/// @notice Fork of RangeVaultArb.sol adding opt-in fee auto-compounding — same
/// feature set as RangeVaultCompound.sol (Celo), ported onto RangeVaultArb.sol's
/// token0/token1-generalized base (stableIsToken0, gasReserveBalance, keeper gas
/// reimbursement). Kept as its own file so neither Celo's RangeVault.sol/
/// RangeVaultCompound.sol nor Arbitrum's plain RangeVaultArb.sol are ever touched.
/// See /Users/elkiyo.eth/.claude/plans/wild-exploring-bumblebee.md for the full
/// design rationale.
contract RangeVaultArbCompound is Initializable, ReentrancyGuardUpgradeable, IERC721Receiver {
    ...
    function withdraw(uint256 positionShareBps, uint256 fundsShareBps) external onlyOwner nonReentrant {
        ...
        // Deliberately excludes fees (never in B1) and reserveShare/
        // gasReserveShare (reserve only ever counts in B1 at the moment it's
        // REINJECTED, per PLAN.md's "la reserva solo cuenta cuando se hace la
        // reinyeccion" — withdrawing it un-reinjected is withdrawing capital
        // that was never added to B1 in the first place, so it can't
        // subtract from it either).
        uint256 principalUsd = _toStableUsd(removedPrincipal0, removedPrincipal1) + investableShare;
    ...
    function withdrawAll() external onlyOwner nonReentrant {
        ...
        // position (removed0/removed1) + all of investableUsdt (100% of it,
        // this is a full exit) — never fees, never un-reinjected reserve.
        uint256 principalUsd = _toStableUsd(removed0, removed1) + investableUsdt;
